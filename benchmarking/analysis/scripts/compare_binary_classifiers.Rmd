...
```{r, include=FALSE}
packages <- c("readr","magrittr","dplyr","ggplot2","stringr","tidyr","knitr","ggrepel","VennDiagram","RColorBrewer","tibble","venn","flextable","patchwork")
...
count_results$software <- c("mlplasmids","PlaScope","Platon","RFPlasmid")
binary_results <- count_results %>% select(TP,TN,FP,FN,precision,recall,f1_score) %>% flextable()
```
```{r}
binary_results
save_as_image(binary_results,"../results/figures/binary_table.png")
```

Show metrics in plot
```{r}
metrics_levels <- c('recall','precision','f1_score')
ggplot(count_results %>% select(software,precision,recall,f1_score) %>% pivot_longer(precision:f1_score, names_to = "metric", values_to = "value"),aes(y=software,x=value))+
  geom_point(size=3,aes(colour=factor(metric,level = metrics_levels)))+
  labs(title="Performance of binary classifiers",y="Software",x="Score",colour="Metric")+
  scale_colour_manual(labels=c("Recall","Precision","F1-Score"),values=c("#4c7af2","#f2924c","#f24c76"))+
  scale_y_discrete(labels=c("mlplasmids","PlaScope","Platon","RFPlasmid"))+
  theme(plot.title = element_text(size = 12))
```

```{r}
ggsave("../results/figures/result_individual.svg",width = 8, height = 2.5)
```
...
VennTP <- venn.diagram(venn_TP,filename = NULL,fill=brewer.pal(4, "Set3"),main = "True Positives",fontfamily = "sans",cat.fontfamily = "sans",main.fontfamily = "sans",main.cex = 1.25,main.pos = c(0.5,1))
```
...
VennTN<- venn.diagram(venn_TN,filename = NULL,fill=brewer.pal(4, "Set3"),main = "True Negatives",fontfamily = "sans",cat.fontfamily = "sans",main.fontfamily = "sans",main.cex = 1.25,main.pos = c(0.5,1))
```

```{r}
venn_FP <- list(mlplasmids = false_positives %>% filter(software=='mlplasmids') %>% pull(contig_name),Platon =false_positives %>% filter(software=='platon') %>% pull(contig_name),PlaScope=false_positives %>% filter(software=='plascope') %>% pull(contig_name),RFPlasmid=false_positives %>% filter(software=='rfplasmid') %>% pull(contig_name))
VennFP<- venn.diagram(venn_FP,filename = NULL,fill=brewer.pal(4, "Set3"),main = "False Positives",fontfamily = "sans",cat.fontfamily = "sans",main.fontfamily = "sans",main.cex = 1.25,main.pos = c(0.5,1))
```

```{r}
venn_FN <- list(mlplasmids = false_negatives %>% filter(software=='mlplasmids') %>% pull(contig_name),Platon =false_negatives %>% filter(software=='platon') %>% pull(contig_name),PlaScope=false_negatives %>% filter(software=='plascope') %>% pull(contig_name),RFPlasmid=false_negatives %>% filter(software=='rfplasmid') %>% pull(contig_name))
VennFN<- venn.diagram(venn_FN,filename = NULL,fill=brewer.pal(4, "Set3"),main = "False Negatives",fontfamily = "sans",cat.fontfamily = "sans",main.fontfamily = "sans",main.cex = 1.25,main.pos = c(0.5,1))
```

```{r}
cowplot::plot_grid(VennTP,VennTN,VennFP,VennFN,labels = c("A","B","C","D"))
```
...
venn_counts <- venn_counts[c("TP","TN","FP","FN"),]
```
...
```{r,echo=FALSE}
ensemble_results <- venn_scores %>% flextable()
```

```{r}
ensemble_results
save_as_image(ensemble_results,"../results/figures/ensemble_table.png")
```
...
count_results_AMR$software <- c("mlplasmids","PlaScope","Platon","RFPlasmid")
binary_results_AMR <- count_results_AMR %>% select(TP,FN,recall) %>% rename(method=software)
```
```{r}
flextable(binary_results_AMR)
```
...
Show diagrams only for Platon, PlaScope and RFPlasmid:
```{r}
#Make a list of all true positives per software
venn_TP_AMR <- list(Platon =true_positives_AMR %>% filter(software=='platon') %>% pull(contig_name),PlaScope=true_positives_AMR %>% filter(software=='plascope') %>% pull(contig_name),RFPlasmid=true_positives_AMR %>% filter(software=='rfplasmid') %>% pull(contig_name))
#Convert list to venn diagram
VennTP_AMR <- venn.diagram(venn_TP_AMR,filename = NULL,fill=brewer.pal(3, "Set3"))
```

```{r}
venn_FN_AMR <- list(Platon =false_negatives_AMR %>% filter(software=='platon') %>% pull(contig_name),PlaScope=false_negatives_AMR %>% filter(software=='plascope') %>% pull(contig_name),RFPlasmid=false_negatives_AMR %>% filter(software=='rfplasmid') %>% pull(contig_name))
VennFN_AMR<- venn.diagram(venn_FN_AMR,filename = NULL,fill=brewer.pal(3, "Set3"))
```

```{r,echo=FALSE}
cowplot::plot_grid(VennTP_AMR,VennFN_AMR,labels = c("True Positives","False Negatives"),label_x = 0.1, label_y=1.1)
```

```{r}
ggsave("../results/figures/venn_AMR_three.svg",width = 10, height =5)
```
...
venn_counts_AMR <- venn_counts_AMR[c("TP","FN"),]
```
...
```{r,echo=FALSE}
venn_scores_AMR %<>% rename(method=Combination)
```

```{r}
all_results_AMR <- flextable(rbind(binary_results_AMR,venn_scores_AMR))
all_results_AMR
```

```{r}
save_as_image(all_results_AMR,"../results/figures/all_results_AMR.png")
```
...
```{r}
ggsave("../results/figures/result_AMR.svg",width = 8, height = 3)
```
