...
```
```{r}
ggsave('contiglengths.png',width = 10,height = 6)
```  
...
Mlplasmids
```{r,message=FALSE}
mlplasmids_out <- read_tsv("../../mlplasmids_bac/results/mlplasmids_all_results.tsv")
...
Platon
```{r}
platon_out <- read.csv("../../platon_bac/results/platon_all_results.csv", header = FALSE)
...
Plascope
```{r}
plascope_out <- read.csv("../../plascope_bac/results/plascope_all_results.csv", header = FALSE)
...
RFPlasmid
```{r}
rfplasmid_out <- read.csv("../../rfplasmid_bac/results/rfplasmid_predictions/prediction.csv")
...
```{r}
false_negatives_venn <- rbind(false_negatives,unclassified %>% filter(classification=="plasmid"))
false_positives_venn <- rbind(false_positives,unclassified %>% filter(classification=="chromosome"))
```
...
rownames(venn_counts) <- c("FN","FP","TP","TN")
...
```{r,echo=FALSE}
kable(venn_scores,caption = "Performance of ensemble classifiers (calculated per contig)")
```
...
kable(count_ensemble_lengths,caption = "Performance of ensemble classifiers (calculated per basepair)")
```


```{r,message=FALSE}
ggplot(lengths_contigs_ensembles %>% filter(result %in% c('FN','FP')),aes(x=log(contig_length,10),fill=result))+
  geom_histogram()+
