...
### Conclusions  
Out of the four softwares, PlaScope calls the least false chromosomes and false plasmids, which results in a very high F1-score of 97.2% (98,9% when counting per basepair). Although this tool scores very favourably for the contigs it assigns to a conclusive category (either plasmid or chromosome), it also assigns 4,4% of contigs to the 'unclassified' category. Because the other tools categorise contigs in a strictly binary matter, comparison to PlaScope based on recall, precision and F1-score is not fair. To compensate for this effect, we can consider all unclassified contigs as incorrect, i.e. we artificially assign the category 'chromosome' to the plasmidal contigs, and 'plasmid' to the chromosomal contigs.
```{r}
...
Mlplasmids has the lowest F1-score, when calculated per contig (83.8%) as well as per basepair (92.1%), mainly due to a low recall (= many false negatives). Platon accomplishes the highest precision of all the tools (contig: 92.3%, bp: 97.0%), while RFplasmid scores the highest recall (contig: 90.4%, bp: 98.0%). Platon also has the highest F1-score for contigs (89.3%). When considering the scores per basepair, PlaScope performs well both in terms of recall and precision, resulting in a top F1-score of 96.8%. However, the lack of conclusive results for some contigs poses a major drawback in using PlaScope as a binary classifier. Based on these results, both Platon or RFPlasmid would be suitable to use as a classifier, depending on whether the focus should be on recall or on precision.
...
rownames(venn_counts2) <- c("FP","TP","TN","FN")
...
kable(venn_scores2)
```
With regard to contigs, the ensemble classifiers outperform all the individual classifiers (top F1-score 89.3%). The combination Platon/PlaScope/RFPlasmid performs best with a recall of 97.7%, precision of 93.1% and F1-score of 95.3%. The highest precision is reached by Mlplasmids/Platon/PlaScope (94.2%).
...
```
Most miscategorised contigs are in the 1000 to 5000 (≈ 10e3.7) bp range. Platon/PlaScope/RFPlasmid misidentifies very few large contigs (>5000 bp), they are all false positives. 
