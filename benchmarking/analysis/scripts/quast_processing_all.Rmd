...
quast_output_all<-rbind(quast_output_mob,quast_output_mob_bac,quast_output_spades)
...
ambiguous_alignments_total<-rbind(ambiguous_alignments_mob,ambiguous_alignments_mob_bac,ambiguous_alignments_spades)
...
```{r}
ggplot(multiple_runs,aes(x=recall_bp,y=precision_bp,colour=sra_accession)) +geom_point() +facet_wrap(~group)
```


```{r}
ggplot(plotting_data,aes(x=recall_bp,y=precision_bp,colour=software)) +geom_point()
```
