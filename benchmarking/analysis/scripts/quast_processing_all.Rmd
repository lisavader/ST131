...
strains_all_replicons <- read.csv("../../../ST131_ncbi_download/results/replicon_data.csv", header = FALSE)
...
not_detected<-quast_output_plasmids %>% filter(software=='not_applicable') %>% select(reference_id) %>% uncount(4) %>% mutate(software = rep(c("MOB-suite (bactofidia)","MOB-suite (unicycler, trimmed)","MOB-suite (unicycler, untrimmed)","PlasmidSPAdes"),times=nrow(not_detected)/4)) %>% mutate(n = 0)
...
Some plotting
```{r}
length_data <- strains_all_replicons %>% filter(classification=="plasmid")
