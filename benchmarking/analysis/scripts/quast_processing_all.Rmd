...
quast_output_mob %<>% mutate(software = "MOB-suite (unicycler)")
...
#MOB_EC
quast_output_mob_ec<-read.csv('../../mobsuite_EC/results/quast_output2/mob_alignments_statistics.csv', sep=',', header=FALSE)
...
#MOB_EC3
quast_output_mob_ec3<-read.csv('../../mobsuite_EC/results/quast_output3/mob_alignments_statistics.csv', sep=',', header=FALSE)
names(quast_output_mob_ec3)<-c('assembly_accession','sra_accession','bin_name_temp','reference_id','alignment_length','bin_length')
quast_output_mob_ec3 %<>% mutate(software = "MOB-suite (unicycler, plasmid contigs)")
#For MOB-suite we will fuse the name of the bin with the name of the strain, to be able to do statistics properly
quast_output_mob_ec3<-unite(quast_output_mob_ec3,'bin_name',c('sra_accession','bin_name_temp'),remove=FALSE) 
#remove old bin name
quast_output_mob_ec3<-quast_output_mob_ec3[,c(2,1,3,5:8)]

#MOB_CLEAN
quast_output_mob_clean<-read.csv('../../mobsuite_clean/results/quast_output_clean2/mob_alignments_statistics.csv', sep=',', header=FALSE)
...
#MOB_CLEAN3
quast_output_mob_clean3<-read.csv('../../mobsuite_clean/results/quast_output_clean3/mob_alignments_statistics.csv', sep=',', header=FALSE)
names(quast_output_mob_clean3)<-c('assembly_accession','sra_accession','bin_name_temp','reference_id','alignment_length','bin_length')
quast_output_mob_clean3 %<>% mutate(software = "MOB-suite (unicycler, cleaned)")
#For MOB-suite we will fuse the name of the bin with the name of the strain, to be able to do statistics properly
quast_output_mob_clean3<-unite(quast_output_mob_clean3,'bin_name',c('sra_accession','bin_name_temp'),remove=FALSE) 
#remove old bin name
quast_output_mob_clean3<-quast_output_mob_clean3[,c(2,1,3,5:8)]
...
```{r}
quast_output_all<-rbind(quast_output_mob,quast_output_mob_bac,quast_output_mob_trimmed,quast_output_spades,quast_output_mob_ec,quast_output_mob_clean,quast_output_mob_clean3,quast_output_mob_ec3)
...
```{r}
assemblies_to_remove <- read.csv("../results/assemblies_in_mobsuite", header = FALSE)
quast_output_filtered <- quast_output_plasmids_chromosomes %>% filter(!assembly_accession %in% assemblies_to_remove$V1)
```
...
ambiguous_alignments_mob$software<-'MOB-suite (unicycler)'
...
names(ambiguous_alignments_mob_ec3)<-c('sra_accession','bin_name_temp','reference_id','contig_name','contig_length','start','end')
ambiguous_alignments_mob_ec3<-unite(ambiguous_alignments_mob_ec3,'bin_name',c('sra_accession','bin_name_temp'),remove=FALSE) 
#remove old bin name
...
names(ambiguous_alignments_mob_clean3)<-c('sra_accession','bin_name_temp','reference_id','contig_name','contig_length','start','end')
ambiguous_alignments_mob_clean3<-unite(ambiguous_alignments_mob_clean3,'bin_name',c('sra_accession','bin_name_temp'),remove=FALSE) 
#remove old bin name
...
ambiguous_alignments_total<-rbind(ambiguous_alignments_mob,ambiguous_alignments_mob_bac,ambiguous_alignments_mob_trimmed,ambiguous_alignments_spades,ambiguous_alignments_mob_ec,ambiguous_alignments_mob_clean,ambiguous_alignments_mob_clean3,ambiguous_alignments_mob_ec3)
...
quast_output_filtered<-left_join(quast_output_filtered,multiple_ambiguous_alignments_final_substracion_data,by=c('sra_accession','bin_name','reference_id','software'))
#replace NA values with 0 in the total_substraction_length (for cases in which there are no multiple alignments)
quast_output_filtered$total_substraction_length[is.na(quast_output_filtered$total_substraction_length)]<-0
...
quast_output_filtered$alignment_length_precision<-(quast_output_filtered$alignment_length-quast_output_filtered$total_substraction_length)
...
ggplot(count_bin_types %>% filter(!software %in% c("MOB-suite (unicycler, trimmed)","MOB-suite (unicycler, cleaned)","MOB-suite (unicycler, plasmid contigs)","MOB-suite (bactofidia, cleaned)","MOB-suite (bactofidia, plasmid contigs)")),aes(y=software,fill=factor(bin_type,level = rev(level_order_types))))+
...
```{r}
ggplot(quast_output_filtered %>% filter())
```
...
```{r}
plotting_data <- filter(quast_output_plasmids ,!software %in% c("PlasmidSPAdes","MOB-suite (unicycler, trimmed)","not_applicable"))
