...
```{r}
quast_output_all<-rbind(quast_output_mob,quast_output_mob_bac,quast_output_mob_trimmed,quast_output_spades)
...
ambiguous_alignments_total<-rbind(ambiguous_alignments_mob,ambiguous_alignments_mob_bac,ambiguous_alignments_mob_trimmed,ambiguous_alignments_spades)
...
ggplot(mixed_bin_analysis,aes(x=chromosomal_content,fill=bin_type))+
  ...
  facet_wrap(~software)
```
Composition of bins containing multiple plasmids
```{r}
multiple_plasmid_bin_analysis <- bin_analysis %>% filter(bin_type=="multiple_plasmids")
multiple_plasmid_bin_analysis %<>% left_join(multiple_ambiguous_alignments_final_substracion_data,by=c('sra_accession','bin_name','reference_id','software'))
#replace NA values with 0 in the total_substraction_length (for cases in which there are no multiple alignments)
multiple_plasmid_bin_analysis$total_substraction_length[is.na(multiple_plasmid_bin_analysis$total_substraction_length)]<-0

#update alignment length
multiple_plasmid_bin_analysis %<>% mutate(actual_alignment_length=alignment_length-total_substraction_length)
multiple_plasmid_bin_analysis %<>% mutate(relative_content=actual_alignment_length/bin_length*100)
```

```{r}
ggplot(multiple_plasmid_bin_analysis,aes(x=relative_content,fill=bin_name))+
  geom_histogram(binwidth = 5,breaks= seq(0,100,by=5))+
  scale_x_continuous(breaks= seq(0,100,by=20))+
  facet_wrap(~software)+
  guides(fill= FALSE)
...
not_detected %<>% mutate(software = rep(c("MOB-suite (bactofidia)","MOB-suite (unicycler, trimmed)","MOB-suite (unicycler, untrimmed)","PlasmidSPAdes"),times=nrow(not_detected)/4)) %>% mutate(n = 0)
...
```{r}
theme_set(theme_bw())
ggplot(bin_number_plotting,aes(x=reference_id,y=n,group=1))+
  geom_point()+
  facet_wrap(~software)
```
...
```{r}
group_A <- quast_output_plasmids %>% filter(assembly_accession=="GCA_002056635.1") %>% mutate(group="A")
group_B <- quast_output_plasmids %>% filter(assembly_accession=="GCA_002180195.1") %>% mutate(group="B")
group_C <- quast_output_plasmids %>% filter(assembly_accession=="GCA_002202175.1") %>% mutate(group="C")
multiple_runs <- rbind(group_A,group_B,group_C)
```

```{r}
ggplot(group_A,aes(x=recall_bp,y=precision_bp,colour=sra_accession)) +geom_count(alpha = 0.5,size =3 ) +
  scale_color_brewer(palette = 'Set1')
ggplot(group_B,aes(x=recall_bp,y=precision_bp,colour=sra_accession)) +geom_count(alpha = 0.5,size =3 ) +
  scale_color_brewer(palette = 'Set1')
ggplot(group_C,aes(x=recall_bp,y=precision_bp,colour=sra_accession)) +geom_count(alpha = 0.5,size =3 ) +
  scale_color_brewer(palette = 'Set1')
```
