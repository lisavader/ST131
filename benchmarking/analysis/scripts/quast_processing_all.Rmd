...
```{r}
level_order_softwares <- c("spades","mob_bac","mob_bac_filtered","mob_bac_cleaned","mob_uni","mob_unitrim","mob_uni_filtered","mob_uni_cleaned")
...
ggplot(count_bin_types, aes(y=factor(software,level=rev(level_order_softwares)),fill=factor(bin_type,level = rev(level_order_types))))+
...
```{r}
ggsave("../results/figures/bin_analysis.svg",width = 8,height = 4)
```
...
```{r}
n_software <- length(level_order_softwares)
...
quast_output_AMR_plasmids <- inner_join(quast_output_plasmids,AMR_replicons,by=c("reference_id"="SEQUENCE"))
```
